...
namespace AssemblyAvalonia.Models.MetaData
{
	public class MetaReader : IMetaFieldVisitor
	{
			...
			LoadType type, FieldChangeSet ignore, FileSegmentGroup segmentGroup)
		{
			...
			_shouldShowDatarefNotice = AppState.Settings.PluginsShowDataRefNotice;
		}
		...
		public void VisitColourFloat(ColorData field)
		{
			...
			{
				float scA = field.Alpha ? _reader.ReadFloat() : 1;
				float scR = _reader.ReadFloat();
				float scG = _reader.ReadFloat();
				float scB = _reader.ReadFloat();
				field.Value = Color.FromArgb(ColorData.FloatToByte(scA), ColorData.LinearToSrgb(scR), ColorData.LinearToSrgb(scG), ColorData.LinearToSrgb(scB));
			}
		}
