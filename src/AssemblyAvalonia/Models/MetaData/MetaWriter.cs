...
namespace AssemblyAvalonia.Models.MetaData
{
	internal class MetaWriter : IMetaFieldVisitor
	{
		...
		public void VisitColourFloat(ColorData field)
		{
			...
			{
				if (field.Alpha)
					_writer.WriteFloat(ColorData.SrgbToLinear(field.Value.A));
				_writer.WriteFloat(ColorData.SrgbToLinear(field.Value.R));
				_writer.WriteFloat(ColorData.SrgbToLinear(field.Value.G));
				_writer.WriteFloat(ColorData.SrgbToLinear(field.Value.B));
			}
		}
